...
function StatCard({ icon: Icon, label, value, color }) {
  return (
    <div className="bg-white dark:bg-surface-900 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
    ...
    <div className="flex items-center gap-3">
      <div className="flex-1">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="h-2 bg-slate-100 dark:bg-surface-900 rounded-full overflow-hidden">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-surface-950 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden animate-scale-in">
        ...
        <div className="overflow-y-auto max-h-[65vh] p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <StatCard icon={Phone} label="Interesados" value={trackedCounts.interested} color="text-brand-500" />
          </div>
          ...
          {trackedCounts.total > 0 && (
            <div>
              ...
              <div className="space-y-3 bg-slate-50 dark:bg-surface-900/50 rounded-xl p-3">
                ...
                <FunnelStep label="Interesados" count={trackedCounts.interested} total={trackedCounts.responded + trackedCounts.interested || 1} color="bg-brand-500" isLast />
              </div>
            </div>
          )}
            ...
            {searchHistory.length === 0 ? (
              ...
              <div className="space-y-1.5">
                {searchHistory.slice(0, 10).map((s, i) => (
                  <div key={i} className="flex items-center justify-between px-3 py-2 bg-white dark:bg-surface-900 rounded-lg border border-slate-100 dark:border-slate-700 text-xs">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className="font-bold text-brand-600 dark:text-brand-400">{s.totalResults}</span>
